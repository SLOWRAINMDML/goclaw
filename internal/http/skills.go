...
func (h *SkillsHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /v1/skills/{id}/feedback", h.authMiddleware(h.handleListFeedback))
	// Skill writes (admin+)
	...
	mux.HandleFunc("POST /v1/skills/{id}/fork", h.adminMiddleware(h.handleFork))
	mux.HandleFunc("POST /v1/skills/{id}/prepare-runtime", h.adminMiddleware(h.handlePrepareRuntime))
	mux.HandleFunc("POST /v1/skills/{id}/feedback", h.adminMiddleware(h.handleAddFeedback))
	// Skill grants (admin+)
