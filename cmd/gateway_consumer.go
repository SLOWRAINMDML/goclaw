...
func consumeInboundMessages(ctx context.Context, msgBus *bus.MessageBus, agents *agent.Router, cfg *config.Config, sched *scheduler.Scheduler, channelMgr *channels.Manager, teamStore store.TeamStore, quotaChecker *channels.QuotaChecker, sessStore store.SessionStore, agentStore store.AgentStore, tenantStore store.TenantStore, providerRegistry *providers.Registry, contactCollector *store.ContactCollector, systemConfigs store.SystemConfigStore, postTurn tools.PostTurnProcessor, subagentMgr *tools.SubagentManager) {
